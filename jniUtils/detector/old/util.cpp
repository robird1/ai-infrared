...
std::vector<int> nms(std::vector<std::vector<float >> boxes, float threshold, std::string type) {
    ...
    std::vector<int> I(boxes.size()), area(boxes.size());
    int n = 0;
    ...
    while (!I.empty()) {
        ...
        pick[counter] = i;
        counter++;
        std::vector<int> xx1(last - 1), yy1(last - 1), xx2(last - 1), yy2(last - 1), w(last - 1), h(
                last - 1);
        n = 0;
...
std::vector<std::vector < float>>
bbreg(std::vector<std::vector<float>>
boundingBox,
std::vector<std::vector<float>> reg
) {
int size = boundingBox.size();
std::vector<float> h(size), w(size);
int n = 0;
std::generate(std::begin(h), std::end(h),
[&]{
