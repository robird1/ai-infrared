...
#ifdef __ANDROID__
    if(m_detectorPfl != nullptr) {
        delete static_cast<detectorPfl *>(m_detectorPfl);
    }
...
#ifdef __ANDROID__
int ulsee::FaceDetectMgr::initDetector(float minSize, AAssetManager * pManager, const char *szModelDir/* = ""*/) {
    std::vector<float> thresholds{0.6, 0.7, 0.5};
    float factor = 0.709;
    ...
    detectorPfl *detPfl = new detectorPfl(minSize, thresholds, factor, szModelDir);

    m_detector = static_cast<void *>(det);
    m_detectorPfl = static_cast<void *>(detPfl);
    return 0;
}
#endif
...
int ulsee::FaceDetectMgr::detectFace(const cv::Mat &img, std::vector<std::vector<float>> & rects, std::vector<std::vector<float>> & points ) {
    if(m_bUsePfl) {
        ...
        detectorPfl * detPfl = static_cast<detectorPfl *>(m_detectorPfl);
        detPfl->detect(img, rects, points);
    } else {
        if(m_detector == nullptr) {
            std::cout<<"please check key or initDetector"<<std::endl;
            return -1;
