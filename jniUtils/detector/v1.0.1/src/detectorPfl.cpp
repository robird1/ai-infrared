...
detectorPfl::detectorPfl(float min_size, std::vector<float> thresholds, float factor,
                         const char *szModelDir)
        : minsize(min_size), thresholds(thresholds), factor(factor) {
    LOGD(TAG,"load_param 0");
    int ret = 0;
    std::string model_path = szModelDir;
    std::vector<std::string> param_files = {
            model_path+"/Pnet.param",
            model_path+"/Rnet.param",
            model_path+"/Onet.param"
    };
    ...
    ret = Pnet.load_param(param_files[0].data());
    LOGD(TAG, "load_param ret=%d", ret);
    ...
    LOGD(TAG, "load_param ret=%d", ret);
    LOGD(TAG, "load_param 1");
}


detectorPfl::~detectorPfl(){
    Pnet.clear();
    Rnet.clear();
    Onet.clear();
}
void detectorPfl::SetMinFace(int minSize){
    minsize = minSize;
}
void detectorPfl::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox>& boundingBox_, float scale){
    const int stride = 2;
...
void detectorPfl::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold, std::string modelname){
...
void detectorPfl::refine(std::vector<Bbox> &vecBbox, const int &height, const int &width, bool square) {
    if(vecBbox.empty()){
...
void detectorPfl::ONet(){
    ...
    for(vector<Bbox>::iterator it=secondBbox_.begin(); it!=secondBbox_.end();it++){
        ...
        LOGD("Extractor 1");
        if (score.channel(1)[0] > thresholds[2]) {
            for(int channel = 0; channel < 4; channel++){
...
void detectorPfl::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_){
    img = img_;
    img_w = img.w;
    img_h = img.h;
...
void detectorPfl::detect(const cv::Mat &img, std::vector<std::vector<float>> &rects,
    ...
    for(Bbox bbox : finalBbox) {
        ...
        rect.push_back((float)bbox.x1);
        rect.push_back((float)bbox.y1);
        rect.push_back((float)bbox.x2);
        rect.push_back((float)bbox.y2);
        rects.push_back(rect);

        std::vector<float> point;
        for(int i = 0; i < 10; i++) {
            point.push_back(bbox.ppoint[i]);
        }
        points.push_back(point);
    }
}

//void detectorPfl::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles){
