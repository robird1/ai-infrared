...
detector::detector(float min_size, std::vector<float> thresholds, float factor,
                    ...
                    : min_size(min_size), thresholds(thresholds), factor(factor) {
...
detector::detect_face(const cv::Mat &iimg) {
    Mat timg;
    ...
    cv::Mat scale_img;
    resize(timg, scale_img, cv::Size(timg.cols * m, timg.rows * m));
    ...
    ncnn::Mat n_input = ncnn::Mat::from_pixels(big_img.data, ncnn::Mat::PIXEL_BGR2RGB, big_img.cols,
                                               big_img.rows);
    ...
    if (!total_boxes.empty()) {
        ...
        std::generate(std::begin(total_tmp), std::end(total_tmp),
                      [&] {
                          ...
                          element.push_back(
                                  total_boxes[pick[n]][0] + total_boxes[pick[n]][5] * regw[n]);
                          element.push_back(
                                  total_boxes[pick[n]][1] + total_boxes[pick[n]][6] * regh[n]);
                          element.push_back(
                                  total_boxes[pick[n]][2] + total_boxes[pick[n]][7] * regw[n]);
                          element.push_back(
                                  total_boxes[pick[n]][3] + total_boxes[pick[n]][8] * regh[n]);
        ...
        pad_res = pad(total_boxes, big_img.rows, big_img.cols);
    }
...
//  total_boxes.clear();
    ...
    for (int i = 0; i < nBox; i++) {
        const int x = pad_res["y"][i];//x1
        const int y = pad_res["x"][i];//y1
        for (int s = 0; s < nScales; s++) {
            const Rect location = location_and_scale[s].first;
            const double scale = location_and_scale[s].second;
            if (location.x < x and x < location.x + location.width and location.y < y and
                y < location.y + location.height) {
                total_boxes[i][1] = pad_res["y"][i] = (pad_res["y"][i] - location.x) / scale / m;
                total_boxes[i][0] = pad_res["x"][i] = (pad_res["x"][i] - location.y) / scale / m;
                total_boxes[i][3] = pad_res["ey"][i] = (pad_res["ey"][i] - location.x) / scale / m;
                total_boxes[i][2] = pad_res["ex"][i] = (pad_res["ex"][i] - location.y) / scale / m;
            }
        }
    }
    ...
    LOGD("stage 1 nms return pick.size[%d].\n", pick.size());
    for (int i = 0; i < pick.size(); i++) {
        total_boxes.push_back(boxs[pick[i]]);
    }

    //update pad_res in original timg coordinates
    pad_res = pad(total_boxes, timg.rows, timg.cols);
#ifdef _SHOW_
...
//   }
    ...
    if (len > 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            ncnn::Mat temp_ncnn = ncnn::Mat::from_pixels(temp.data, ncnn::Mat::PIXEL_BGR2RGB,
                                                         temp.cols, temp.rows);
...
//                mv_ele.push_back(bbox.channel(3)[0]);
                mv_ele.push_back(bbox.channel(0)[0]);
                mv_ele.push_back(bbox.channel(0)[1]);
                mv_ele.push_back(bbox.channel(0)[2]);
                mv_ele.push_back(bbox.channel(0)[3]);
                mv.push_back(mv_ele);
            }
        ...
        if (len > 0) {
            ...
            pad_res = pad(finer_boxes, timg.rows, timg.cols);
            temp_imgs.clear();
            for (int i = 0; i < len; i++) {
                Mat temp(Size(pad_res["tmph"][i], pad_res["tmpw"][i]), CV_8UC3, CV_RGB(0, 0, 0));
                Rect r(pad_res["y"][i] - 1, pad_res["x"][i] - 1,
...
//                                  [&] { return keyPoint.channel(n++)[0]; });
                                  [&] { return keyPoint.channel(0)[n++]; });

                    std::vector<float> temp(finer_boxes[i].begin(),
                                            finer_boxes[i].begin() + 4), mv_ele;
