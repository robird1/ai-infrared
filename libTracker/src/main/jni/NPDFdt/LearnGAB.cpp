...
vector<int> GAB::DetectFace(Mat img, vector<Rect>& rects, vector<float>& scores){
	...
	//double dTimeCost = double(clock() - StartTime) / CLOCKS_PER_SEC;
	//printf("time cost: %f FPS\n", 1.0 / dTimeCost);


	int minFace = 20;// 20;
	int maxFace = ((img.cols+img.rows)>>1);// 3000;
...
//	omp_set_num_threads(opt.numThreads);
	...
	for (int k = 0; k < 29; k++) // process each scale
	{
		...
		int winStep = (int)floor(pWinSize[k] * 0.05);
	...
	for (int i = 0; i<picked.size(); i++){
		...
		*/
		...
		int y0 = rects[idx].y;
		int x0 = max(rects[idx].x - 0.1*Srect[idx], 0);
		int height = Srect[idx] * 1.25;
		int width = Srect[idx] * 1.2;
		...
		if (rects[idx].y + height - 1>imgHeight - 1)
		{
			rects[idx].height = imgHeight - rects[idx].y - 2;
		}
		if (rects[idx].x + width - 1>imgWidth - 1)
		{
			rects[idx].width = imgWidth - rects[idx].x - 2;
		}
		/*  rects[idx].y = y0;
		rects[idx].x = x0;
...
//  omp_set_num_threads(opt.numThreads);
...
#else
  Mat tmpim = img.t();
  memcpy(I, tmpim.data, width*height);
#endif
  ...
  if (!m_binit)
  {
	  ...
	  m_binit = true;
	  for (int k = 0; k < 29; k++){
		  int twinsize = pWinSize[k];
		  m_intbuffer[k] = new int[twinsize * twinsize];

		  int pp1 = 0, pp2 = 0, gap = height - twinsize;
		  for (int j = 0; j < twinsize; j++) { // column coordinate		
			  for (int i = 0; i < twinsize; i++) { // row coordinate			 
				  m_intbuffer[k][pp1++] = pp2++;
			  }
			  pp2 += gap;
		  }
	  }
  ...
  int *ptable = new int[width*height];
  memset(ptable, 0, sizeof(int)*width*height);
  ...
  for(int k = 0; k < 29; k++) // process each scale
  {
	  ...
	  if (twinsize < minFace) continue;
	  else if (twinsize > maxFace) break;

      // determine the step of the sliding subwindow
	  //int winStep = (int)floor(twinsize * 0.1);
	  //if (twinsize > 40) winStep = (int)floor(twinsize * 0.05);

#if 1
	  int winStep = (twinsize * 0.1);
	  if (twinsize > 48) winStep = (twinsize * 0.05);
#else
	  int winStep = (twinsize * 0.05);
#endif

	  //int winStep = (twinsize * 0.05);
	  //if ((m_flag % 3) != 0){
		 // winStep = (twinsize * 0.1);
		 // if (twinsize > 48) winStep = (twinsize * 0.05);
	  //}
    ...
    // pre-determined offset of pixels in a subwindow
#if 0
	  vector<int> offset(twinsize * twinsize);
	  int pp1 = 0, pp2 = 0, gap = height - twinsize;
	  for (int j = 0; j < twinsize; j++){ // column coordinate
		for (int i = 0; i < twinsize; i++){ // row coordinate
        offset[pp1++] = pp2++;
...
#endif
	  int colMax = width - twinsize + 1;
	  int rowMax = height - twinsize + 1;

	  int *tptr = m_intbuffer[k];
    // process each subwindow
    #pragma omp parallel for
    for(int c = 0; c < colMax; c += winStep) // slide in column
    {
      ...
      for(int r = 0; r < rowMax; r += winStep, pPixel += winStep) // slide in row
      {
...
#if 0
		int cc = 0;
		for (int m = 0; m < twinsize; m++){
			for (int n = 0; n < twinsize; n++){
				if (ptable[(c + m)*height + r + n] > 0){
					cc++;
				}
			}
		}
		//printf("local candiates: %d\n", cc);

		if (cc>10){//ptable[c*height + r] > 0){
		//	//printf("check\n");
			//c += winStep;
			r += (twinsize>>1);
			pPixel += (twinsize>>1);
        ...
        for(s = 0; s < stages; s++)
        {
          ...
          while(node > -1) // branch node
          {
			  int tt1 = tptr[points1[k][node]];// offset[points1[k][node]];
			  int tt2 = tptr[points2[k][node]];// offset[points2[k][node]];

            unsigned char p1 = pPixel[ tt1 ];
            unsigned char p2 = pPixel[ tt2 ];
            unsigned char fea = ppNpdTable.at<uchar>(p1,p2);
        ...
        if(s == stages) // a face detected
        {
			...
			//printf("check: %d\n", ptable[c*height + r]);
			Rect roi(c, r, twinsize, twinsize);
          #pragma omp critical // modify the record by a single thread
          {
            rects.push_back(roi);
            scores.push_back(_score);
		  ...
		  timecost += double(clock() - StartTime) / CLOCKS_PER_SEC;
...
#if 0
	char szName[256];
	sprintf(szName, "d:/_table_%d.txt", k);
	FILE *fp = fopen(szName, "w");
	int cccount = 0;
	for (int c = 0; c < width; c++) // slide in column
	{
		for (int r = 0; r < height; r++) // slide in row
		{
			if (ptable[c*height + r] > 0){
				cccount++;
			}
			fprintf(fp, "%d", ptable[c*height + r]);
		}
		fprintf(fp, "\n");
	}
	fclose(fp);
  ...
  //printf("time cost in stages processing: %f sec\n", timecost);
  //FILE *fp = fopen("d:/checktable.txt", "w");

  //int cccount = 0;
  //for (int c = 0; c < height; c++) // slide in column
  //{
	 // for (int r = 0; r < width; r++) // slide in row
	 // {
		//  if (ptable[c*height + r] > 0)
		//	  cccount++;
		//  //fprintf(fp, "%d ", ptable[c*height + r]);
  ...
  //double dTimeCost = double(clock() - StartTime) / CLOCKS_PER_SEC;
  //printf("time cost in second part: %f sec\n", dTimeCost);
  ...
  //dTimeCost = double(clock() - StartTime) / CLOCKS_PER_SEC;
  //printf("time cost of Nms: %f sec\n", dTimeCost);
...
vector<int> GAB::Nms(vector<Rect>& rects, vector<float>& scores, vector<int>& Srect, float overlap, Mat Img) {
	...
	int i, j, w, h, s, th, tw, wxh;
  int numCandidates = rects.size();
  Mat predicate = Mat::eye(numCandidates,numCandidates,IPL_DEPTH_1U);
  for( i = 0;i<numCandidates;i++)
  {
	  th = rects[i].y + rects[i].height;
	  tw = rects[i].x + rects[i].width;
	  wxh = rects[i].width*rects[i].height;

    for( j = i+1;j<numCandidates;j++){
       h = min(th,rects[j].y+rects[j].height) - max(rects[i].y,rects[j].y);
       w = min(tw,rects[j].x+rects[j].width) - max(rects[i].x,rects[j].x);
       s = max(h,0)*max(w,0);

      if ((float)s/(float)(wxh+rects[j].width*rects[j].height-s)>=overlap){
