...
size_t ZSTD_decodeSeqHeaders(ZSTD_DCtx *dctx, int *nbSeqPtr,
    ...
    *nbSeqPtr = nbSeq;
    ...
    RETURN_ERROR_IF(ip + 1 > iend,
                    srcSize_wrong); /* minimum possible size: 1 byte for symbol encoding types */
    {
...
size_t ZSTD_execSequence(BYTE *op,
                         BYTE *const oend, seq_t sequence,
                         const BYTE **litPtr, const BYTE *const litLimit,
                         const BYTE *const prefixStart, const BYTE *const virtualStart,
                         const BYTE *const dictEnd) {
    BYTE *const oLitEnd = op + sequence.litLength;
    size_t const sequenceLength = sequence.litLength + sequence.matchLength;
    BYTE *const oMatchEnd = op + sequenceLength;   /* risk : address space overflow (32-bits) */
    ...
    if (oLitEnd > oend_w)
        return ZSTD_execSequenceLast7(op, oend, sequence, litPtr, litLimit, prefixStart,
                                      virtualStart, dictEnd);

    /* copy Literals */
    if (sequence.litLength > 8)
        ZSTD_wildcopy_16min(op, (*litPtr), sequence.litLength,
                            ZSTD_no_overlap);   /* note : since oLitEnd <= oend-WILDCOPY_OVERLENGTH, no risk of overwrite beyond oend */
    else
    ...
    *litPtr = iLitEnd;   /* update for next sequence */
    ...
    if (oMatchEnd > oend - (16 - MINMATCH)) {
        ...
        ZSTD_wildcopy(op, match, (ptrdiff_t) sequence.matchLength - 8,
                      ZSTD_overlap_src_before_dst);   /* works even if matchLength < 8 */
    }
    return sequenceLength;
}
...
size_t ZSTD_execSequenceLong(BYTE *op,
                             BYTE *const oend, seq_t sequence,
                             const BYTE **litPtr, const BYTE *const litLimit,
                             const BYTE *const prefixStart, const BYTE *const dictStart,
                             const BYTE *const dictEnd) {
    BYTE *const oLitEnd = op + sequence.litLength;
    size_t const sequenceLength = sequence.litLength + sequence.matchLength;
    BYTE *const oMatchEnd = op + sequenceLength;   /* risk : address space overflow (32-bits) */
    ...
    RETURN_ERROR_IF(iLitEnd > litLimit, corruption_detected, "over-read beyond lit buffer");
    if (oLitEnd > oend_w)
        return ZSTD_execSequenceLast7(op, oend, sequence, litPtr, litLimit, prefixStart, dictStart,
                                      dictEnd);
...
typedef enum {
    ZSTD_lo_isRegularOffset, ZSTD_lo_isLongOffset = 1
} ZSTD_longOffset_e;
    ...
    /* sequence */
    {
        ...
        if (!ofBits)
            ...
            if (MEM_32bits() && longOffsets && (ofBits >= STREAM_ACCUMULATOR_MIN_32)) {
                ...
                offset = ofBase + BIT_readBitsFast(&seqState->DStream,
...
ZSTD_decompressSequences_body(ZSTD_DCtx *dctx,
    ...
    if (nbSeq) {
        ...
        {
            U32 i;
            for (i = 0; i < ZSTD_REP_NUM; i++) seqState.prevOffset[i] = dctx->entropy.rep[i];
        }
        RETURN_ERROR_IF(
        ...
        for (; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) && nbSeq;) {
            nbSeq--;
            {
                ...
                size_t const oneSeqSize = ZSTD_execSequence(op, oend, sequence, &litPtr, litEnd,
...
ZSTD_decodeSequenceLong(seqState_t *seqState, ZSTD_longOffset_e const longOffsets) {
    ...
    /* sequence */
    {
        ...
        if (!ofBits)
            ...
            if (MEM_32bits() && longOffsets) {
                ...
                offset = ofBase + BIT_readBitsFast(&seqState->DStream,
                                                   ofBits);   /* <=  (ZSTD_WINDOWLOG_MAX-1) bits */
                if (MEM_32bits()) BIT_reloadDStream(&seqState->DStream);
            }
        }
    ...
    seq.matchLength = mlBase + ((mlBits > 0) ? BIT_readBitsFast(&seqState->DStream, mlBits)
                                             : 0);  /* <=  16 bits */
    if (MEM_32bits() &&
        (mlBits + llBits >= STREAM_ACCUMULATOR_MIN_32 - LONG_OFFSETS_MAX_EXTRA_BITS_32))
        BIT_reloadDStream(&seqState->DStream);
...

    {
        ...
        const BYTE *const matchBase = (seq.offset > pos) ? seqState->dictEnd
                                                         : seqState->prefixStart;
...
#define ADVANCED_SEQS 4
        ...
        for (; (BIT_reloadDStream(&(seqState.DStream)) <= BIT_DStream_completed) &&
            ...
            size_t const oneSeqSize = ZSTD_execSequenceLong(op, oend,
                                                            sequences[(seqNb - ADVANCED_SEQS) &
                                                                      STORED_SEQS_MASK], &litPtr,
        ...
        for (; seqNb < nbSeq; seqNb++) {
            size_t const oneSeqSize = ZSTD_execSequenceLong(op, oend,
                                                            ...
                                                            &litPtr, litEnd, prefixStart, dictStart,
                                                            dictEnd);
            if (ZSTD_isError(oneSeqSize)) return oneSeqSize;
            op += oneSeqSize;
        }
...
ZSTD_decompressBlock_internal(ZSTD_DCtx *dctx,
                              ...
                              const void *src, size_t srcSize,
                              const int frame) {   /* blockType == blockCompressed */
     ...
     */
    ZSTD_longOffset_e const isLongOffset = (ZSTD_longOffset_e) (MEM_32bits() && (!frame ||
    ...
    !defined(ZSTD_FORCE_DECOMPRESS_SEQUENCES_LONG)
        if (!usePrefetchDecoder
            ...
            U32 const minShare = MEM_64bits() ? 7
                                              : 20; /* heuristic values, correspond to 2.73% and 7.81% */
