...
size_t ZSTD_compressBlock_doubleFast_generic(
    ...
    const U32 prefixLowestIndex = (endIndex - lowestValid > maxDistance) ? endIndex - maxDistance
                                                                         : lowestValid;
    ...
    while (ip < ilimit) {   /* < instead of <=, because repcode check at (ip+1) */
...

        {
            ...
            if (matchIndexL3 > prefixLowestIndex) {
                ...
                if (dictMatchL3 > dictStart && MEM_read64(dictMatchL3) == MEM_read64(ip + 1)) {
                    mLength = ZSTD_count_2segments(ip + 1 + 8, dictMatchL3 + 8, iend, dictEnd,
                                                   prefixLowest) + 8;
                    ip++;
        ...
        ZSTD_storeSeq(seqStore, (size_t) (ip - anchor), anchor, offset + ZSTD_REP_MOVE,
                      mLength - MINMATCH);
                ...
                while (ip <= ilimit) {
                    ...
                    if (((U32) ((prefixLowestIndex - 1) - (U32) repIndex2) >=
                        ...
                        size_t const repLength2 =
                                ZSTD_count_2segments(ip + 4, repMatch2 + 4, iend, repEnd2,
                                                     prefixLowest) + 4;
...
size_t ZSTD_compressBlock_doubleFast(
    ...
    switch (mls) {
        ...
        case 4 :
            return ZSTD_compressBlock_doubleFast_generic(ms, seqStore, rep, src, srcSize, 4,
                                                         ZSTD_noDict);
        case 5 :
            return ZSTD_compressBlock_doubleFast_generic(ms, seqStore, rep, src, srcSize, 5,
                                                         ZSTD_noDict);
        case 6 :
            return ZSTD_compressBlock_doubleFast_generic(ms, seqStore, rep, src, srcSize, 6,
                                                         ZSTD_noDict);
        case 7 :
            return ZSTD_compressBlock_doubleFast_generic(ms, seqStore, rep, src, srcSize, 7,
                                                         ZSTD_noDict);
    }
}
...
static size_t ZSTD_compressBlock_doubleFast_extDict_generic(
    ...
    if (prefixStartIndex == dictStartIndex)
        return ZSTD_compressBlock_doubleFast_generic(ms, seqStore, rep, src, srcSize, mls,
                                                     ZSTD_noDict);
...
size_t ZSTD_compressBlock_doubleFast_extDict(
    ...
    switch (mls) {
        default: /* includes case 3 */
        case 4 :
            return ZSTD_compressBlock_doubleFast_extDict_generic(ms, seqStore, rep, src, srcSize,
                                                                 4);
        case 5 :
            return ZSTD_compressBlock_doubleFast_extDict_generic(ms, seqStore, rep, src, srcSize,
                                                                 5);
