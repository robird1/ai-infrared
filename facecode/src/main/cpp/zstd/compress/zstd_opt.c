        ...
        if (optPtr->symbolCosts->huf.repeatMode == HUF_repeat_valid) {
            ...
            if (compressedLiterals) {
                ...
                for (lit = 0; lit <= MaxLit; lit++) {
                    ...
                    optPtr->litFreq[lit] = bitCost ? 1 << (scaleLog - bitCost)
...

            {
                ...
                for (ll = 0; ll <= MaxLL; ll++) {
                    ...
                    assert(bitCost < scaleLog);
                    optPtr->litLengthFreq[ll] = bitCost ? 1 << (scaleLog - bitCost)
                                                        : 1 /*minimum to calculate cost*/;
...

            {
                ...
                for (ml = 0; ml <= MaxML; ml++) {
                    ...
                    assert(bitCost < scaleLog);
                    optPtr->matchLengthFreq[ml] = bitCost ? 1 << (scaleLog - bitCost)
                                                          : 1 /*minimum to calculate cost*/;
...

            {
                ...
                for (of = 0; of <= MaxOff; of++) {
                    ...
                    assert(bitCost < scaleLog);
                    optPtr->offCodeFreq[of] = bitCost ? 1 << (scaleLog - bitCost)
                                                      : 1 /*minimum to calculate cost*/;
            ...
            if (compressedLiterals) {
                unsigned lit = MaxLit;
                HIST_count_simple(optPtr->litFreq, &lit, src,
...
static int
ZSTD_litLengthContribution(U32 const litLength, const optState_t *const optPtr, int optLevel) {
    ...
    /* dynamic statistics */
    {
        ...
        int const contribution = (int) (LL_bits[llCode] * BITCOST_MULTIPLIER)
                                 + (int) WEIGHT(optPtr->litLengthFreq[0],
                                                optLevel)   /* note: log2litLengthSum cancel out */
...
ZSTD_getMatchPrice(U32 const offset,
    ...
    if ((optLevel < 2) /*static*/ && offCode >= 20)
        price += (offCode - 19) * 2 *
                 BITCOST_MULTIPLIER; /* handicap for long distance offsets, favor decompression speed */

    /* match Length */
    {
        U32 const mlCode = ZSTD_MLcode(mlBase);
        price += (ML_bits[mlCode] * BITCOST_MULTIPLIER) + (optPtr->matchLengthSumBasePrice -
                                                           WEIGHT(optPtr->matchLengthFreq[mlCode],
                                                                  optLevel));
    }
    ...
    while (nbCompares-- && (matchIndex >= windowLow)) {
        ...
        if (!extDict || (matchIndex + matchLength >= dictLimit)) {
            assert(matchIndex + matchLength >=
                   dictLimit);   /* might be wrong if actually extDict */
...
U32 ZSTD_insertBtAndGetAllMatches(
    ...
    U32 matchEndIdx = current + 8 +
                      1;   /* farthest referenced position of any match => detects repetitive patterns */
    U32 dummy32;   /* to be nullified at the end */
    U32 mnum = 0;
    U32 nbCompares = 1U << cParams->searchLog;
    ...
    U32 const dmsBtLow = dictMode == ZSTD_dictMatchState && dmsBtMask < dmsHighLimit - dmsLowLimit ?
                         dmsHighLimit - dmsBtMask : dmsLowLimit;
    ...
    assert(ll0 <= 1);   /* necessarily 1 or 0 */
    {
        ...
        for (repCode = ll0; repCode < lastR; repCode++) {
            ...
            if (repOffset - 1 /* intentional overflow, discards 0 and -1 */ <
                ...
                if (ZSTD_readMINMATCH(ip, minMatch) ==
                ...
                if (dictMode == ZSTD_extDict
                    ...
                    repLen = (U32) ZSTD_count_2segments(ip + minMatch, repMatch + minMatch, iLimit,
                                                        dictEnd, prefixStart) + minMatch;
                }
                if (dictMode == ZSTD_dictMatchState
                    && (((repOffset - 1) /*intentional overflow*/ < current - (dmsLowLimit +
                                                                               dmsIndexDelta))  /* equivalent to `current > repIndex >= dmsLowLimit` */
                        & ((U32) ((dictLimit - 1) - repIndex) >=
                           3)) /* intentional overflow : do not test positions overlapping 2 memory segments */
                    && (ZSTD_readMINMATCH(ip, minMatch) == ZSTD_readMINMATCH(repMatch, minMatch))) {
                    repLen = (U32) ZSTD_count_2segments(ip + minMatch, repMatch + minMatch, iLimit,
                                                        dmsEnd, prefixStart) + minMatch;
                }
            }
    ...
    if ((mls == 3) /*static*/ && (bestLength < mls)) {
        ...
        if ((matchIndex3 >= matchLow)
            ...
            if ((dictMode == ZSTD_noDict) /*static*/ ||
                (dictMode == ZSTD_dictMatchState) /*static*/ || (matchIndex3 >= dictLimit)) {
    ...
    while (nbCompares-- && (matchIndex >= matchLow)) {
        ...
        if ((dictMode == ZSTD_noDict) || (dictMode == ZSTD_dictMatchState) ||
            ...
            assert(matchIndex + matchLength >=
                   dictLimit);  /* ensure the condition is correct when !extDict */
            match = base + matchIndex;
            if (matchIndex >= dictLimit)
                assert(memcmp(match, ip, matchLength) ==
        ...
        while (nbCompares-- && (dictMatchIndex > dmsLowLimit)) {
            ...
            if (dictMatchIndex + matchLength >= dmsHighLimit)
                match = base + dictMatchIndex +
                        dmsIndexDelta;   /* to prepare for next usage of match[matchLength] */
...
FORCE_INLINE_TEMPLATE U32 ZSTD_BtGetAllMatches(
    ...
    switch (matchLengthSearch) {
        case 3 :
            return ZSTD_insertBtAndGetAllMatches(matches, ms, nextToUpdate3, ip, iHighLimit,
                                                 dictMode, rep, ll0, lengthToBeat, 3);
        default :
        case 4 :
            return ZSTD_insertBtAndGetAllMatches(matches, ms, nextToUpdate3, ip, iHighLimit,
                                                 dictMode, rep, ll0, lengthToBeat, 4);
        case 5 :
            return ZSTD_insertBtAndGetAllMatches(matches, ms, nextToUpdate3, ip, iHighLimit,
                                                 dictMode, rep, ll0, lengthToBeat, 5);
        case 7 :
        case 6 :
            return ZSTD_insertBtAndGetAllMatches(matches, ms, nextToUpdate3, ip, iHighLimit,
                                                 dictMode, rep, ll0, lengthToBeat, 6);
    }
}
...
ZSTD_compressBlock_opt_generic(ZSTD_matchState_t *ms,
    ...
    while (ip < ilimit) {
        ...
        /* find first match */
        {
            ...
            U32 const nbMatches = ZSTD_BtGetAllMatches(matches, ms, &nextToUpdate3, ip, iend,
                                                       dictMode, rep, ll0, minMatch);
            if (!nbMatches) {
                ip++;
                continue;
            }
            ...
            {
                U32 i;
                for (i = 0; i < ZSTD_REP_NUM; i++) opt[0].rep[i] = rep[i];
            }
            ...
            /* set prices for first matches starting position == 0 */
            {
                U32 const literalsPrice =
                        opt[0].price + ZSTD_litLengthPrice(0, optStatePtr, optLevel);
                U32 pos;
                U32 matchNb;
                ...
                for (matchNb = 0; matchNb < nbMatches; matchNb++) {
                    ...
                    for (; pos <= end; pos++) {
                        U32 const matchPrice = ZSTD_getMatchPrice(offset, pos, optStatePtr,
                                                                  optLevel);
        ...
        for (cur = 1; cur <= last_pos; cur++) {
...

            {
                ...
                U32 const nbMatches = ZSTD_BtGetAllMatches(matches, ms, &nextToUpdate3, inr, iend,
                                                           dictMode, opt[cur].rep, ll0, minMatch);
                U32 matchNb;
                if (!nbMatches) {
                ...
                for (matchNb = 0; matchNb < nbMatches; matchNb++) {
                    ...
                    for (mlen = lastML; mlen >= startML; mlen--) {  /* scan downward */
                        ...
                        int const price =
                                basePrice + ZSTD_getMatchPrice(offset, mlen, optStatePtr, optLevel);
            ...
            DEBUGLOG(6, "sending selected sequences into seqStore")
            {
                ...
                for (storePos = storeStart; storePos <= storeEnd; storePos++) {
                    ...
                    if (mlen ==
                        ...
                        ip = anchor +
                             llen;     /* last "sequence" is a bunch of literals => don't progress anchor */
                    ...
                    if (offCode >= ZSTD_REP_NUM) {  /* full offset */
                        ...
                        if (repCode) {  /* note : if repCode==0, no change */
                            U32 const currentOffset = (repCode == ZSTD_REP_NUM) ? (rep[0] - 1)
                                                                                : rep[repCode];
...
size_t ZSTD_compressBlock_btopt(
    ...
    return ZSTD_compressBlock_opt_generic(ms, seqStore, rep, src, srcSize, 0 /*optLevel*/,
                                          ZSTD_noDict);
}
...
ZSTD_initStats_ultra(ZSTD_matchState_t *ms,
    ...
    ZSTD_compressBlock_opt_generic(ms, seqStore, tmpRep, src, srcSize, 2 /*optLevel*/,
                                   ZSTD_noDict);   /* generate stats into ms->opt*/
...
size_t ZSTD_compressBlock_btultra(
    ...
    return ZSTD_compressBlock_opt_generic(ms, seqStore, rep, src, srcSize, 2 /*optLevel*/,
                                          ZSTD_noDict);
}
