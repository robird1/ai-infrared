...
ZSTD_updateDUBT(ZSTD_matchState_t *ms,
    ...
    if (idx != target) DEBUGLOG(7, "ZSTD_updateDUBT, from %u to %u (dictLimit:%u)",
...
ZSTD_insertDUBT1(ZSTD_matchState_t *ms,
    ...
    U32 const windowLow = (current - windowValid > maxDistance) ? current - maxDistance
                                                                : windowValid;
    ...
    while (nbCompares-- && (matchIndex > windowLow)) {
        ...
        size_t matchLength = MIN(commonLengthSmaller,
        ...
        if ((dictMode != ZSTD_extDict)
            ...
            const BYTE *const mBase = ((dictMode != ZSTD_extDict)
                                      ...
                                      base : dictBase;
            assert((matchIndex + matchLength >=
                    dictLimit)   /* might be wrong if extDict is incorrectly set to 0 */
                   || (current < dictLimit));
...
ZSTD_DUBT_findBetterDictMatch(
    ...
    U32 const btMask = (1 << btLog) - 1;
    U32 const btLow = (btMask >= dictHighLimit - dictLowLimit) ? dictLowLimit : dictHighLimit -
                                                                                btMask;
    ...
    while (nbCompares-- && (dictMatchIndex > dictLowLimit)) {
        ...
        matchLength += ZSTD_count_2segments(ip + matchLength, match + matchLength, iend, dictEnd,
                                            prefixStart);
        if (dictMatchIndex + matchLength >= dictHighLimit)
            match = base + dictMatchIndex +
                    dictIndexDelta;   /* to prepare for next usage of match[matchLength] */

        if (matchLength > bestLength) {
            U32 matchIndex = dictMatchIndex + dictIndexDelta;
            if ((4 * (int) (matchLength - bestLength)) >
                (int) (ZSTD_highbit32(current - matchIndex + 1) -
                ...
                DEBUGLOG(9,
                         ...
                         current, (U32) bestLength, (U32) matchLength, (U32) *offsetPtr,
                         ZSTD_REP_MOVE + current - matchIndex, dictMatchIndex, matchIndex);
                bestLength = matchLength, *offsetPtr = ZSTD_REP_MOVE + current - matchIndex;
            }
            if (ip + matchLength ==
                iend) {   /* reached end of input : ip[matchLength] is not valid, no way to know if it's larger or smaller than match */
    ...
    /* find longest match */
    {
        ...
        while (nbCompares-- && (matchIndex > windowLow)) {
            ...
            if ((dictMode != ZSTD_extDict) || (matchIndex + matchLength >= dictLimit)) {
                ...
                if (matchIndex + matchLength >= dictLimit)
                    match = base +
                            matchIndex;   /* to prepare for next usage of match[matchLength] */
            }
...
size_t ZSTD_HcFindBestMatch_generic(
    ...
    const U32 withinMaxDistance = (current - lowestValid > maxDistance) ? current - maxDistance
                                                                        : lowestValid;
    ...
    for (; (matchIndex > lowLimit) & (nbAttempts > 0); nbAttempts--) {
        ...
        if ((dictMode != ZSTD_extDict) || matchIndex >= dictLimit) {
            ...
            assert(matchIndex >=
                   dictLimit);   /* ensures this is true if dictMode != ZSTD_extDict */
            if (match[ml] == ip[ml])   /* potentially better */
                currentMl = ZSTD_count(ip, match, iLimit);
        } else {
            const BYTE *const match = dictBase + matchIndex;
            assert(match + 4 <= dictEnd);
            if (MEM_read32(match) == MEM_read32(
...
*********************************/
typedef enum {
    search_hashChain, search_binaryTree
} searchMethod_e;
...
ZSTD_compressBlock_lazy_generic(
    ...
    searchMax_f const searchMax = dictMode == ZSTD_dictMatchState ?
                                  (searchMethod == search_binaryTree
        ...
        if (depth >= 1)
            while (ip < ilimit) {
                ...
                if ((dictMode == ZSTD_noDict)
                    ...
                    int const gain1 = (int) (matchLength * 3 - ZSTD_highbit32((U32) offset + 1) +
                                             1);
                ...
                if (dictMode == ZSTD_dictMatchState) {
                    ...
                    const BYTE *repMatch = repIndex < prefixLowestIndex ?
                                           dictBase + (repIndex - dictIndexDelta) :
                                           base + repIndex;
                    if (((U32) ((prefixLowestIndex - 1) - repIndex) >=
                ...
                }
                {
                    ...
                    int const gain1 = (int) (matchLength * 4 - ZSTD_highbit32((U32) offset + 1) +
                                             4);
                ...
                if ((depth == 2) && (ip < ilimit)) {
                    ...
                    }
                    {
                        size_t offset2 = 999999999;
                        size_t const ml2 = searchMax(ms, ip, iend, &offset2);
                        int const gain2 = (int) (ml2 * 4 - ZSTD_highbit32(
                                (U32) offset2 + 1));   /* raw approx */
                        int const gain1 = (int) (matchLength * 4 -
                                                 ZSTD_highbit32((U32) offset + 1) + 7);
                        if ((ml2 >= 4) && (gain2 > gain1)) {
                            matchLength = ml2, offset = offset2, start = ip;
                            continue;
                        }
            ...
            if (dictMode == ZSTD_noDict) {
                while (((start > anchor) & (start - (offset - ZSTD_REP_MOVE) > prefixLowest))
                       && (start[-1] == (start - (offset -
                                                  ZSTD_REP_MOVE))[-1]))  /* only search for offset within prefix */
                {
                    start--;
                    matchLength++;
                }
        ...
        if (dictMode == ZSTD_dictMatchState) {
            while (ip <= ilimit) {
                ...
                const BYTE *repMatch = dictMode == ZSTD_dictMatchState
                                       ...
                                       base + repIndex;
                if (((U32) ((prefixLowestIndex - 1) - (U32) repIndex) >=
...
size_t ZSTD_compressBlock_btlazy2(
    ...
    return ZSTD_compressBlock_lazy_generic(ms, seqStore, rep, src, srcSize, search_binaryTree, 2,
                                           ZSTD_noDict);
}
...
size_t ZSTD_compressBlock_lazy2(
        ZSTD_matchState_t *ms, seqStore_t *seqStore, U32 rep[ZSTD_REP_NUM],
        void const *src, size_t srcSize) {
    return ZSTD_compressBlock_lazy_generic(ms, seqStore, rep, src, srcSize, search_hashChain, 2,
                                           ZSTD_noDict);
}
...
size_t ZSTD_compressBlock_greedy(
        ZSTD_matchState_t *ms, seqStore_t *seqStore, U32 rep[ZSTD_REP_NUM],
        void const *src, size_t srcSize) {
    return ZSTD_compressBlock_lazy_generic(ms, seqStore, rep, src, srcSize, search_hashChain, 0,
                                           ZSTD_noDict);
}
...
size_t ZSTD_compressBlock_lazy_extDict_generic(
    ...
    searchMax_f searchMax =
            searchMethod == search_binaryTree ? ZSTD_BtFindBestMatch_extDict_selectMLS
    ...
    while (ip < ilimit) {
        ...
        /* check repCode */
        {
            ...
            if (((U32) ((dictLimit - 1) - repIndex) >= 3) &
                ...
                if (MEM_read32(ip + 1) == MEM_read32(repMatch)) {
                    ...
                    matchLength = ZSTD_count_2segments(ip + 1 + 4, repMatch + 4, iend, repEnd,
                                                       prefixStart) + 4;
                    if (depth == 0) goto _storeSequence;
                }
        }
        ...
        if (offset) {
            U32 const matchIndex = (U32) ((start - base) - (offset - ZSTD_REP_MOVE));
            const BYTE *match = (matchIndex < dictLimit) ? dictBase + matchIndex : base +
                                                                                   matchIndex;
