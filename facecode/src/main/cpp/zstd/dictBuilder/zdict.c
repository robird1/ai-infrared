...
static dictItem ZDICT_analyzePos(
...

    {
        ...
        DISPLAYLEVEL(4, "Selected dict at position %u, of length %u : saves %u (ratio: %.2f)  \n",
                     (unsigned) pos, (unsigned) maxLength, (unsigned) savings[maxLength],
...
static U32 ZDICT_tryMerge(dictItem *table, dictItem elt, U32 eltNbToSkip, const void *buffer) {
    ...
    for (u = 1; u < tableSize; u++) {
        ...
        if ((table[u].pos + table[u].length >= elt.pos) &&
            (table[u].pos < elt.pos)) {  /* overlap, existing < new */
            /* append */
...
static size_t ZDICT_trainBuffer_legacy(dictItem *dictList, U32 dictListSize,
    ...
    BYTE *doneMarks = (BYTE *) malloc(
            (bufferSize + 16) * sizeof(*doneMarks));   /* +16 for overflow security */
...
#   define DISPLAYUPDATE(l, ...) if (notificationLevel>=l) { \
    ...
    {
        int const divSuftSortResult = divsufsort((const unsigned char *) buffer, suffix,
                                                 (int) bufferSize, 0);
...
static void ZDICT_countEStats(EStats_ress_t esr, ZSTD_parameters params,
                              unsigned *countLit, unsigned *offsetcodeCount,
                              unsigned *matchlengthCount, unsigned *litlengthCount, U32 *repOffsets,
...
static size_t ZDICT_analyzeEntropy(void *dstBuffer, size_t maxDstSize,
    ...
    esr.dict = ZSTD_createCDict_advanced(dictBuffer, dictBufferSize, ZSTD_dlm_byRef,
    ...
    /* analyze, build stats, starting with literals */
    {
        ...
        if (maxNbBits == 8) {  /* not compressible : will fail on HUF_writeCTable() */
            ...
            ZDICT_flatLit(
                    countLit);  /* replace distribution by a fake "mostly flat but still compressible" distribution, that HUF_writeCTable() can encode */
    ...
    total = 0;
    for (u = 0; u <= MaxML; u++) total += matchLengthCount[u];
    ...
    total = 0;
    for (u = 0; u <= MaxLL; u++) total += litLengthCount[u];
    ...
    {
        size_t const ohSize = FSE_writeNCount(dstPtr, maxDstSize, offcodeNCount, OFFCODE_MAX,
                                              Offlog);
...
#define HBUFFSIZE 256   /* should prove large enough for all entropy headers */
    ...
    int const compressionLevel = (params.compressionLevel == 0) ? g_compressionLevel_default
...
static size_t ZDICT_addEntropyTablesFromBuffer_advanced(
    ...
    DISPLAYLEVEL(2, "statistics ... \n");
    {
        size_t const eSize = ZDICT_analyzeEntropy((char *) dictBuffer + hSize,
                                                  ...
                                                  dictContentSize, dictContentSize,
                                                  notificationLevel);
        if (ZDICT_isError(eSize)) return eSize;
        hSize += eSize;
    }
    ...
    {
        U64 const randomID = XXH64((char *) dictBuffer + dictBufferCapacity - dictContentSize,
                                   dictContentSize, 0);
        U32 const compliantID = (randomID % ((1U << 31) - 32768)) + 32768;
        U32 const dictID = params.dictID ? params.dictID : compliantID;
...
size_t ZDICT_trainFromBuffer_unsafe_legacy(
    ...
    unsigned const selectivity =
            params.selectivityLevel == 0 ? g_selectivity_default : params.selectivityLevel;
    ...
    /* create dictionary */
    {
        ...
        if ((dictContentSize > targetDictSize * 3) && (nbSamples > 2 * MINRATIO) &&
            (selectivity > 1)) {
...
size_t ZDICT_trainFromBuffer_legacy(void *dictBuffer, size_t dictBufferCapacity,
    ...
    ZDICT_fillNoise((char *) newBuff + sBuffSize,
                    NOISELENGTH);   /* guard band, for end of buffer condition */
