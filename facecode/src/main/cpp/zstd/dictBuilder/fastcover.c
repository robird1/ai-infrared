...
static COVER_segment_t FASTCOVER_selectSegment(const FASTCOVER_ctx_t *ctx,
    ...
    while (activeSegment.end < end) {
        ...
        if (activeSegment.end - activeSegment.begin == dmersInK + 1) {
            ...
            const size_t delIndex = FASTCOVER_hashPtrToIndex(ctx->samples + activeSegment.begin, f,
                                                             d);
...
static void FASTCOVER_tryParameters(void *opaque) {
    ...
    /* Build the dictionary */
    {
        ...
        const unsigned nbFinalizeSamples = (unsigned) (ctx->nbTrainSamples *
                                                       ctx->accelParams.finalize / 100);
        selection = COVER_selectDict(dict + tail, dictBufferCapacity - tail,
                                     ctx->samples, ctx->samplesSizes, nbFinalizeSamples,
...
ZDICT_trainFromBuffer_fastCover(void *dictBuffer, size_t dictBufferCapacity,
    ...
    /* Initialize context */
    {
        size_t const initVal = FASTCOVER_ctx_init(&ctx, samplesBuffer, samplesSizes, nbSamples,
                                                  coverParams.d, parameters.splitPoint,
                                                  parameters.f,
                                                  accelParams);
